...
# Endpoint para solicitar dados dos filmes baseado em filtros de busca e paginação
# Padrão: Retorna a primeira página sem filtro de busca
@router.get("/")
async def get_films(request: Request, search: str = "", page: str = "", order_by: str = "", reverse: bool = False):
    if page:
        return await helpers.get_from_url(f"https://swapi.dev/api/films/?search={search}&page={page}", request, order_by, reverse)
    return await helpers.get_all_from_pages(f"https://swapi.dev/api/films/?search={search}", request, order_by, reverse)
